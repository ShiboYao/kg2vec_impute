...
def train(model, optimizer, epoch, idx_train, adj, batch_sz = 32):
    ...
    for i in range(0, idx_train.size()[0], batch_sz):
        ...
        optimizer.zero_grad()
        loss_train = mse_loss(output, word_vector[indices])
        loss_train.backward()
        optimizer.step()
...
def test(model, adj, features, word2node, word2id, emb, dataset):
    model.eval()
    output = model(features, torch.arange(features.size()[0])).detach().cpu().numpy()
    if dataset == "rw":
...
if __name__ == "__main__":
    ...
    adj, features, word_vector, word2node, idx_train = build_graph(
            args.wv+'_'+args.dataset, word2id, emb, json_folder, json_oov)
