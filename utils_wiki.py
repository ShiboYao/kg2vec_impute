...
def build_graph(data, word2id, emb, json_folder, json_oov):
    ...
    features = np.array(feats_all)
    wv_all = np.array(wv_all)
    idx_train = np.array(range(train_num))  

    tmp = [adj, features, wv_all, word2node, idx_train]
    with open(data+'.pkl', 'wb') as handle:
        pkl.dump(tmp, handle, protocol=pkl.HIGHEST_PROTOCOL)
    '''
    with open(data+'.pkl', 'rb') as handle:
        adj, features, wv_all, word2node, idx_train = pkl.load(handle)
    ...
    wv_all = torch.FloatTensor(wv_all)
